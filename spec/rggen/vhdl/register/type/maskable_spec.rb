...
RSpec.describe 'register/type/maskable' do
  ...
  describe '#generate_code' do
    ...
    it 'rggen_maskable_registerをインスタンスするコードを出力する' do
      expect(registers[0]).to generate_code(:register, :top_down, <<~"CODE")
        ...
        u_register: entity #{library_name}.rggen_maskable_register
          generic map (
            READABLE        => true,
            WRITABLE        => true,
            ADDRESS_WIDTH   => 9,
      ...
      expect(registers[5]).to generate_code(:register, :top_down, <<~"CODE")
        \\g_tie_off\\: for \\__i\\ in 0 to 31 generate
          g: if (bit_slice(x"0000f0f0", \\__i\\) = '0') generate
            bit_field_read_data(\\__i\\) <= '0';
            bit_field_value(\\__i\\) <= '0';
          end generate;
        end generate;
      ...
      expect(registers[7]).to generate_code(:register, :top_down, <<~"CODE")
        \\g_tie_off\\: for \\__i\\ in 0 to 63 generate
          g: if (bit_slice(x"000000f0000000f0", \\__i\\) = '0') generate
            bit_field_read_data(\\__i\\) <= '0';
            bit_field_value(\\__i\\) <= '0';
          end generate;
        end generate;
